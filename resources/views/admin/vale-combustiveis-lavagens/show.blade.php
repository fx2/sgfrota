...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb back-transparente">
            ...
            <li class="breadcrumb-item active" aria-current="page">Visualizar Valecombustiveislavagens</li>
        </ol>
    </nav> 
    <div class="card">
        <div class="card-header">ValeCombustiveisLavagen </div>
        <div class="card-body">
            ...
            <a href="{{ url('/vale-combustiveis-lavagens/' . $result->id . '/edit') }}" title="Edit ValeCombustiveisLavagen"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

            <form method="POST" action="{{ url('valecombustiveislavagens' . '/' . $result->id) }}" accept-charset="UTF-8" style="display:inline">
                {{ method_field('DELETE') }}
                {{ csrf_field() }}
                <button type="submit" class="btn btn-danger btn-sm" title="Delete ValeCombustiveisLavagen" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
            </form>
            <br/>
            <br/>
